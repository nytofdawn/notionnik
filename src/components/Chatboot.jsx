...
function RobotSVG({ walkFrame, hovered, idle, facingLeft }) {
  ...
  const pointArm   = hovered ? -40 : armSwing; // point up when hovered
  ...
  return (
    ...
    >
      ...
      {/* ── LEFT ARM (pointing up when hovered) ── */}
      <g transform={`translate(6, 58) rotate(${pointArm - 5}, 7, 0)`}>
        ...
        {/* Finger (points up when hovered) */}
        {hovered && (
          <rect x="5.5" y="20" width="3" height="10" rx="1.5" fill="url(#bodyGrad)" />
        )}
        {/* Hand shine */}
      ...
      {/* ── SMILE ── */}
      <path
        d={hovered ? "M 30 42 Q 40 48 50 42" : "M 31 42 Q 40 47 49 42"}
...
function RobotBot({ onOpen, hasUnread }) {
  ...
  const currentLine = ROBOT_LINES[lineIdx];
    ...
    >
      ...
      {(showBubble || hovered) && (
        <div style={{
          ...
          {hovered ? "Click to chat! 💬" : currentLine}
          <div style={{
      ...
      {isOpen && (
        <div className="fixed bottom-5 right-5 z-[99990] w-80 flex flex-col rounded-2xl overflow-hidden shadow-2xl border border-zinc-700/60" style={{ height: "420px" }}>
          ...
          <div className="bg-zinc-950 border-b border-zinc-800 px-4 py-2.5 flex items-center justify-between shrink-0">
            ...
            <button onClick={() => setIsOpen(false)} className="w-6 h-6 rounded-lg bg-zinc-800 border border-zinc-700 text-zinc-400 hover:text-white hover:bg-zinc-700 transition-colors flex items-center justify-center text-xs">✕</button>
          </div>
          ...
          {messages.length <= 1 && !isSending && (
            <div className="flex flex-wrap gap-1.5 px-3 py-2 bg-zinc-900 border-t border-zinc-800 shrink-0">
              {QUICK_REPLIES.map((qr) => (
                <button key={qr} onClick={() => handleSend(qr)} className="text-[10px] px-2.5 py-1 rounded-full border border-zinc-700 text-zinc-400 hover:border-sky-500 hover:text-sky-300 hover:bg-sky-500/10 transition-all">{qr}</button>
              ))}
            </div>
          )}
          ...
          <div className="bg-zinc-950 border-t border-zinc-800 px-2.5 py-2 flex items-center gap-2 shrink-0">
            <input
              ...
              maxLength={500}
            />
            <button onClick={() => handleSend()} disabled={isSending || !input.trim()} className="w-8 h-8 shrink-0 bg-gradient-to-br from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 rounded-xl flex items-center justify-center text-white shadow-md transition-all hover:scale-105 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100">
